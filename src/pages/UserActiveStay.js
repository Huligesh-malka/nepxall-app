...
const UserActiveStay = () => {
  ...
  const submitVacateRequest = async (stayId) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // Close the form and reset fields BEFORE loadStay
        ...
        setIfscCode("");
        setUpiId("");
        // Reload so stay.vacate_status is now "pending" from API
        await loadStay(false);
      }
    } catch (err) {
      console.error(err);
  ...
  const VacateStatusBanner = ({ stay }) => {
    ...
    const config = statusConfig[stay.vacate_status] || statusConfig.pending;
      ...
      >
        <div
          style={{
            ...
            alignItems: "center",
            gap: "10px",
            marginBottom: stay.vacate_date ? "8px" : "0",
          }}
        >
          ...
          <div>
            ...
            >
              🚪 Vacate Request: {config.label}
            </p>
            {stay.vacate_date && (
              <p style={{ fontSize: "12px", color: "#666", margin: "4px 0 0 0" }}>
                Requested Vacate Date: {formatDate(stay.vacate_date)}
              </p>
            )}
          </div>
        </div>

        {/* Show refund info inside vacate status if available */}
        {stay.refund_status && (
          <div
            style={{
              marginTop: "12px",
            ...
            {stay.refund_status === "approved" &&
              stay.user_approval === "pending" && (
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <button
                    style={{ ...btn, background: "#4CAF50", flex: "none", padding: "8px 16px" }}
                    onClick={() => acceptRefund(stay.id)}
                  >
                    ✅ Accept
                  </button>
                  <button
                    style={{ ...btn, background: "#ef4444", flex: "none", padding: "8px 16px" }}
  ...
  return (
    <div style={container}>
      ...
      {stays.map((stay) => {
        // KEY LOGIC: vacate_status from API is the single source of truth.
        // If ANY vacate_status exists → only show status banner, never the form.
        const hasVacateRequest = !!stay.vacate_status;

        return (
          <div key={stay.id} style={card}>

            {/* ── VACATE STATUS BANNER (shown whenever vacate was submitted) ── */}
            {hasVacateRequest && <VacateStatusBanner stay={stay} />}

            {/* ── VACATE FORM (only if no vacate request at all) ── */}
            {!hasVacateRequest && showVacateFormFor === stay.id ? (
              <div style={refundFormContainer}>
                <h3 style={{ color: "#f59e0b", marginBottom: "20px" }}>🚪 Vacate Request</h3>
                ...
                <div style={inputGroup}>
                  ...
                  <textarea
                    style={{ ...inputField, minHeight: "80px", resize: "vertical" }}
