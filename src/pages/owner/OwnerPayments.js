...
import {
  ...
  Modal, Fade, Checkbox, TextField, Backdrop, IconButton, Divider, Alert, Stack, Grid, Card, CardContent, Collapse, useMediaQuery, useTheme
} from "@mui/material";
import { 
...
export default function OwnerPayments() {
  ...
  const getPGStats = (bookings) => {
    const totalAmount = bookings.reduce(
      (sum, item) => sum + (parseFloat(item.owner_amount) || 0),
      0
    );
  ...
  return (
    <Container sx={{ py: { xs: 2, md: 4 }, maxWidth: '1400px', px: { xs: 1, md: 2 } }}>
      <Box display="flex" justifyContent="space-between" mb={3} alignItems="center" flexWrap="wrap" gap={2}>
        <Typography variant="h4" fontWeight="bold" fontSize={{ xs: '1.5rem', md: '2rem' }}>
          <span style={{ color: BRAND_BLUE }}>Nep</span>
          <span style={{ color: BRAND_GREEN }}>xall</span>
          <span style={{ color: BRAND_DARK, fontSize: '0.8em', marginLeft: '8px' }}>Owner Settlement</span>
        </Typography>
        <Button onClick={fetchPayments} startIcon={<Refresh />} variant="outlined" sx={{ borderRadius: 2, px: 3, py: { xs: 0.5, md: 1 } }}>
          Refresh Data
        </Button>
      </Box>
          ...
          <TableBody>
            {data.length === 0 ? (
              ...
              Object.keys(groupedData).map((pgName) => {
                ...
                const isExpanded = expandedPGs[pgName] !== false;
                    ...
                    >
                      ...
                      <TableCell colSpan={7} sx={{ fontWeight: "bold", py: 1.5 }}>
                        <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
                          ...
                          <Box display="flex" gap={1} sx={{ ml: { md: 'auto' }, flexWrap: 'wrap' }}>
                            <Chip label={`${stats.total} Booking${stats.total !== 1 ? 's' : ''}`} size="small" variant="outlined" sx={{ bgcolor: 'white' }} />
                            <Chip label={`₹${stats.totalAmount.toLocaleString()}`} size="small" color="primary" sx={{ fontWeight: 'bold' }} />
                            <Chip label={`${stats.paidCount} Paid`} size="small" color="success" />
                            {stats.pendingCount > 0 && <Chip label={`${stats.pendingCount} Pending`} size="small" color="warning" />}
                          </Box>
                        </Box>
                      </TableCell>
                    </TableRow>
                        ...
                        <Collapse in={isExpanded} timeout="auto" unmountOnExit>
                          <Box sx={{ margin: 0 }}>
                            {groupedData[pgName].map(item => {
                              ...
                              const showDraftButton = item.final_pdf && item.final_pdf !== "0" && !item.signed_pdf;
                              const showSignButton = item.viewed_by_owner === true || item.viewed_by_owner === 1;
      ...
      >
        <Fade in={openReceiptModal}>
          <Box sx={{
            ...
            <Box ref={receiptRef} sx={{ p: { xs: 2, md: 4 }, bgcolor: '#ffffff' }}>
              <Box sx={{ textAlign: 'center', mb: 4 }}>
                <Typography variant="h2" fontWeight="800" sx={{ letterSpacing: 2, fontSize: { xs: '2rem', md: '2.5rem' } }}>
                  <span style={{ color: BRAND_BLUE }}>Nep</span>
                  <span style={{ color: BRAND_GREEN }}>xall</span>
                </Typography>
            ...
            <Box sx={{ p: 2.5, bgcolor: '#f8f9fa', borderTop: '1px solid #e0e0e0', display: 'flex', gap: 2 }}>
              <Button fullWidth variant="outlined" onClick={() => setOpenReceiptModal(false)} sx={{ borderRadius: 2, py: 1 }}>
                Close
              </Button>
              <Button fullWidth variant="contained" onClick={downloadPDF} startIcon={<Download />} sx={{ borderRadius: 2, py: 1, bgcolor: BRAND_BLUE, '&:hover': { bgcolor: '#1d4ed8' } }}>
                Download PDF
              </Button>
            </Box>
          </Box>
      ...
      >
        <Fade in={openSignModal}>
          <Box sx={{
            ...
            <Box display="flex" alignItems="center" mb={2} justifyContent="space-between" flexWrap="wrap">
            ...
            {step === 1 && (
              <Box>
                ...
                <Button fullWidth variant="contained" size="large" disabled={!agreed} onClick={() => setStep(2)} sx={{ py: 1.5, fontWeight: 'bold', borderRadius: 2, bgcolor: BRAND_BLUE, '&:hover': { bgcolor: '#1d4ed8' } }}>
            ...
            {step === 2 && (
              <Box py={2} textAlign="center">
                ...
                {!confirmObj ? (
                  ...
                  <>
                    <TextField fullWidth label="6-Digit OTP" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} sx={{ mb: 3 }} inputProps={{ maxLength: 6 }} />
