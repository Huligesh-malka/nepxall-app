  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(auth, async (fbUser) => {
      ...
      try {
        ...
        /* get pg id from backend context */
        if (userRes.data.pg_id) {
          setPgId(userRes.data.pg_id);
        }
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        ...
        <div style={styles.headerInfo}>
          ...
          <div style={styles.status}>{online ? "🟢 online" : "⚪ offline"}</div>
        </div>
...
header:{background:"linear-gradient(135deg,#667eea,#764ba2)",color:"#fff",padding:"12px 15px",display:"flex",alignItems:"center",gap:10},
...
bubble:{padding:"10px 14px",borderRadius:15,maxWidth:"70%",background:"#fff"},
deleteBtn:{marginLeft:10,cursor:"pointer"},
inputArea:{display:"flex",padding:10,background:"#fff"},
input:{flex:1,padding:10,borderRadius:20,border:"1px solid #ddd"},
sendBtn:{marginLeft:10,padding:"0 16px",background:"#667eea",color:"#fff",border:"none",borderRadius:20},
