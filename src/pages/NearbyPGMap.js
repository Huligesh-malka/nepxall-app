    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // UPDATED ENDPOINT: Using /api/nearby-pg to avoid route collision
          ...
          {selectedPG && (
            ...
            >
              <div style={{ maxWidth: 220 }}>
                ...
                <p>₹ {selectedPG.price || selectedPG.rent_amount || "Contact"}</p>
              </div>
            </InfoWindow>
          )}
        </GoogleMap>
        ...
        <div style={styles.grid}>
          {pgs.map((pg) => {
            ...
            try {
              ...
              image = pg.image || "https://via.placeholder.com/400x250?text=No+Image";
            }
              ...
              <div key={pg.id} style={styles.card}>
                ...
                <div style={styles.content}>
                  <div style={{ marginBottom: 10 }}>
                    {pg.source === "website" ? (
                      <span style={styles.websiteBadge}>Verified by Nepxall</span>
                    ) : (
                      <span style={styles.googleBadge}>Google Nearby</span>
                    )}
                  </div>

                  <h2 style={styles.name}>{pg.pg_name || pg.name}</h2>
                  ...
                  <div style={styles.buttonContainer}>
                    {pg.source === "website" ? (
                      ...
                      <a
                        ...
                        style={{ textDecoration: "none" }}
                      >
                        <button style={styles.bookBtn}>View Place</button>
                      </a>
                    )}
                    ...
                    <a
                      ...
                      style={{ textDecoration: "none" }}
                    >
                      <button style={styles.navBtn}>Navigate</button>
                    </a>
                  </div>
                </div>
              </div>
...
const styles = {
  ...
  heading: { textAlign: "center", marginBottom: 20, color: "#0B5ED7" },
  ...
  card: { background: "#fff", borderRadius: 15, overflow: "hidden", boxShadow: "0 4px 15px rgba(0,0,0,0.08)" },
  image: { width: "100%", height: 220, objectFit: "cover" },
  content: { padding: 15 },
  name: { margin: 0, marginBottom: 10, color: "#222" },
  price: { color: "#4CAF50", fontWeight: "bold", fontSize: 18 },
  address: { color: "#666", marginBottom: 10 },
  distance: { color: "#0B5ED7", fontWeight: "600" },
  buttonContainer: { display: "flex", gap: 10, marginTop: 15, flexWrap: "wrap" },
  bookBtn: { background: "#25D366", color: "#fff", border: "none", padding: "10px 15px", borderRadius: 8, cursor: "pointer", fontWeight: "bold" },
  navBtn: { background: "#0B5ED7", color: "#fff", border: "none", padding: "10px 15px", borderRadius: 8, cursor: "pointer", fontWeight: "bold" },
  websiteBadge: { background: "#E8F5E9", color: "#2E7D32", padding: "5px 10px", borderRadius: 20, fontSize: 12, fontWeight: "bold" },
  googleBadge: { background: "#FFF3E0", color: "#EF6C00", padding: "5px 10px", borderRadius: 20, fontSize: 12, fontWeight: "bold" },
};
