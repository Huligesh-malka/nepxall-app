...
function AdminAddPG() {
  ...
  const getMissingFieldsList = () => {
    ...
    // PHOTOS ARE OPTIONAL FOR ADMIN - removed from required check
  ...
  const validateForm = () => {
    ...
    // PHOTOS ARE OPTIONAL FOR ADMIN - removed validation
    if (form.contact_email?.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.contact_email)) {
  ...
  const handleSubmit = async () => {
    ...
    if (missingFields.length > 0) {
      ...
      if (!form.contact_person?.trim()) errors.contact_person = true;
      if (!form.contact_phone?.trim()) errors.contact_phone = true;
      // PHOTOS ARE OPTIONAL FOR ADMIN - removed error highlighting
      if (form.contact_email?.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.contact_email)) {
        errors.contact_email = true;
      }
      if (form.contact_phone?.trim() && !/^[0-9]{10,15}$/.test(form.contact_phone.replace(/\D/g, ''))) {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        <h2 style={styles.title}>👑 Admin Add Property</h2>
        <div style={styles.premiumBanner}>
          🛠️ Admin Access: Property Management
          <br />
          <span style={{ fontSize: "13px", opacity: 0.9 }}>
            Photos are optional • Full control over all properties
          </span>
        </div>
        ...
        {/* Photo Upload - OPTIONAL for Admin */}
        <div style={styles.section}>
          <h3 style={styles.sectionTitle}>📷 Property Photos <span style={{ fontSize: "12px", color: "#666", fontWeight: "normal" }}>(Optional for Admin)</span></h3>
          <p style={styles.note}>Photos are optional. Maximum 10 photos allowed.</p>
          ...
          {/* No validation error for photos - admin can skip */}
        </div>
...
const styles = {
  ...
  premiumBanner: { background: "linear-gradient(135deg, #FFF3E0, #FFE0B2)", padding: "15px", borderRadius: "12px", textAlign: "center", marginBottom: "25px", color: "#E65100", fontSize: "14px", fontWeight: "500" },
