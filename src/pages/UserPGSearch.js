...
function UserPGSearch() {
  ...
  // OPTIMIZED: Fetch full data only on Quick View click
  const openQuickView = async (pgId, e) => {
    e.stopPropagation();
    
    try {
      setQuickViewLoading(true);
      const res = await api.get(`/pg/quick-view/${pgId}`);
      setQuickViewPG(res.data.data);
    } catch (err) {
      console.log("Error loading property details:", err);
      showNotification("Failed to load property details");
    } finally {
  ...
  return (
    <div style={{ padding: 20, maxWidth: 1400, margin: "auto", minHeight: "100vh" }}>
      ...
      {/* Loading Overlay for Quick View */}
      {quickViewLoading && <LoadingOverlay />}
            ...
            {pgs.map((pg) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    onClick={(e) => openQuickView(pg.id, e)}
                    style={{
                      position: "absolute",
                      top: 12,
                      right: 12,
